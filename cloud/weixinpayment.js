...
AV.Cloud.define("PaymentSendRefundRequest", function (request, response) {
	...
	shippingQuery.get(shippingId).then(function(shipping){
			...
			else if(shipping.get("paymentStatus") != messageModule.PF_SHIPPING_PAYMENT_STATUS_SUCCESS())
			{
...
AV.Cloud.define("PaymentChargeShippingListCancel", function (request, response) {
	...
	shippingQuery.get(shippingId).then(function(shipping){
			...
			else if(shipping.get("paymentStatus") != messageModule.PF_SHIPPING_PAYMENT_STATUS_PENDING())
			{
			    response.error({code: 102, message: "状态不对"});
			}
